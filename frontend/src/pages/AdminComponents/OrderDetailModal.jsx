    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl border border-white/8 bg-background p-6 shadow-2xl">
        ...
        <div className="mb-6">
          ...
          <div className="space-y-2 bg-secondary rounded-2xl p-4">
            {items.length > 0 ? (
              items.map((item, idx) => (
                <div key={idx} className="flex items-center justify-between gap-4 pb-2 border-b border-border last:border-0 last:pb-0">
        ...
        {/* Pricing Summary */}
        <div className="mb-6 bg-secondary rounded-2xl p-4 space-y-2">
            ...
            {shipping.email && (
              <div className="bg-secondary rounded-2xl p-4 flex items-center justify-between">
                ...
                <button
                  ...
                  className="ml-2 p-2 hover:bg-black/5 rounded-lg transition-colors shrink-0"
                >
                  {copiedText === 'email' ? (
          ...
          <div className="bg-secondary rounded-2xl p-4 space-y-3 border border-border">
            ...
            {payment.razorpayPaymentId && (
              <div className="flex justify-between gap-4 items-center">
                ...
                <span className="font-mono text-xs font-semibold text-emerald-700 text-right break-all">{payment.razorpayPaymentId}</span>
              </div>
            )}
            {payment.paidAt && (
              <div className="flex justify-between gap-4 items-center">
            ...
            <div className="flex justify-between items-center pt-2 border-t border-border">
              ...
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${payment.verified || payment.status === 'paid' ? 'bg-emerald-500' : 'bg-amber-500'}`} />
                <span className={`font-semibold text-sm ${payment.verified || payment.status === 'paid' ? 'text-emerald-700' : 'text-amber-700'}`}>
            ...
            {(payment.screenshotUrl || payment.screenshot || payment.paymentScreenshotUrl) && (
              <div className="pt-2">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="block overflow-hidden rounded-xl border border-border bg-secondary hover:border-primary/30 transition-colors"
                >
                  <img
          ...
          <div className="mb-6 bg-red-50 border border-red-200 rounded-2xl p-4">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div className="flex-1">
                <h4 className="font-semibold text-red-800 mb-2">Cancel Order</h4>
                ...
                <textarea
                  ...
                  className="w-full bg-white border border-red-200 rounded-lg p-2 text-sm text-foreground placeholder-secondary-text focus:outline-none focus:ring-2 focus:ring-red-400 resize-none"
                  rows="3"
                />
                {cancelError && (
                ...
                <div className="flex gap-3 mt-4">
                  ...
                  <button
                    ...
                    disabled={isCanceling}
                    className="flex-1 px-3 py-2 rounded-full border border-border bg-secondary text-foreground text-sm font-semibold hover:bg-accent disabled:opacity-60 transition-colors"
                  >
                    Keep Order
                  </button>
                </div>
        ...
        <div className="flex gap-2 mb-3">
          <button
            ...
            className="flex-1 px-4 py-2 rounded-full bg-sky-100 text-sky-800 border border-sky-300 font-semibold text-sm hover:bg-sky-200 transition-colors flex items-center justify-center gap-2"
          >
          ...
          <button
            ...
            className="flex-1 px-4 py-2 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-300 font-semibold text-sm hover:bg-emerald-200 transition-colors flex items-center justify-center gap-2"
          >
            ...
            <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 space-y-3">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className={`flex-1 px-4 py-2 rounded-full font-semibold transition-colors ${payment.verified ? 'bg-emerald-600 text-white' : 'bg-emerald-100 text-emerald-800 border border-emerald-300 hover:bg-emerald-200'}`}
                >
                ...
                <button
                  ...
                  className={`flex-1 px-4 py-2 rounded-full font-semibold transition-colors ${!payment.verified ? 'bg-amber-600 text-white' : 'bg-amber-50 text-amber-800 border border-amber-300 hover:bg-amber-100'}`}
                >
                  Mark Unverified
                </button>
              </div>
          ...
          <div className="flex gap-3">
            {canCancel && !showCancelPrompt && (
              <button
                ...
                className="flex-1 px-4 py-3 rounded-full bg-red-50 text-red-700 border border-red-200 font-semibold hover:bg-red-100 transition-colors"
              >
                Cancel Order
              </button>
            )}
